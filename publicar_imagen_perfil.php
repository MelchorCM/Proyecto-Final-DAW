...
<body>
    ...
    $ruta_imagen_perfil_antigua = "./imagenes_perfil/$imagen_perfil_antigua";  // Ruta y nombre del archivo que deseas eliminar
        ...
        if (empty($_FILES['imagen']['name'])) {
            ...
            if (is_uploaded_file($_FILES['imagen']['tmp_name'])) {
                ...
                if (strstr($tipo, "image")) {
                    ...
                    if (file_exists($ruta_imagen_perfil_antigua)) {
                        //Elimina la foto antigua
                        unlink($ruta_imagen_perfil_antigua); 
                    }
                    if (@move_uploaded_file($_FILES['imagen']['tmp_name'], "imagenes_perfil/" . $nombre_imagen)) {
                        // Se ha movido correctamente
                        // Vamos a hacer un enlace y a mostrar la imagen
                        // El link
                        echo "<center><a href='imagenes_perfil/$nombre_imagen' target='_blank'>Ver la imagen en otra pestaña</a><br>";
                        echo "<img src='imagenes_perfil/$nombre_imagen'>";
                    } else {
            ...
            $insercion = "UPDATE usuarios SET imagen_perfil = '$nombre_imagen' WHERE id = '$usuario_id'";
